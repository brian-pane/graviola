...
pub(crate) fn sha512_compress_blocks(state: &mut [u64; 8], blocks: &[u8]) {
    ...
    if let Some(token) = x86_64::cpu::HaveSha512::check() {
        x86_64::sha512_ext::sha512_compress_blocks_shaext(state, blocks, token)
    } else if let Some(token) = x86_64::cpu::HaveBmi2::check() {
